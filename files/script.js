...
document.addEventListener('click', function (e) {
  ...
  if (e.target.id === 'needHelpModal') closeNeedHelpModal();
...
/*
  ...
  policies). This sends a postMessage command to the YouTube
  iframe to unmute/mute on tap, without needing the full YT API.
*/
var _heroMuted = true;
...
/* ── Need Help? — gated WhatsApp flow ──────────────────────── */
/*
  Instead of an instant WhatsApp button (which generates a lot
  of low-quality "how much / what is this" enquiries), visitors
  first see quick answers to the most common questions. Only
  after reading those do they see the WhatsApp CTA. This keeps
  WhatsApp reachable in one extra tap, but nudges genuinely
  interested buyers to convert directly, and pre-answers casual
  questions for everyone else.
*/
function openNeedHelpModal() {
  fbqTrack('Contact', { content_name: 'Need Help Widget Opened' });
  var overlay = document.getElementById('needHelpModal');
  if (!overlay) return;
  overlay.classList.add('active');
  document.body.style.overflow = 'hidden';
}
...
function goToWhatsAppFromHelp() {
  fbqTrack('Contact', { content_name: 'WhatsApp Click — Need Help Widget' });
  var text = encodeURIComponent("Hi! I checked the FAQ but still have a question about the Flute Mastery course.");
  window.open('https://wa.me/' + WHATSAPP_NUM + '?text=' + text, '_blank', 'noopener');
  closeNeedHelpModal();
}
